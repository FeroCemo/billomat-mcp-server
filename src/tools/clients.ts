...
export function registerClientTools(server: McpServer, api: BillomatClient): void {
    server.tool(
        "list-clients",
        ...
        async (params) => {
            ...
            const response = await api.get("/clients", p as Record<string, unknown>);
            const clients = toArray(response?.clients?.client);
            return { content: [{ type: "text", text: JSON.stringify(clients, null, 2) }] };
        }
    );

    server.tool("get-client", "Get a Billomat client by ID",
